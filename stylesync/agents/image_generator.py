...
def _get_pipeline() -> TryOnPipeline:
    ...
    if _pipeline is None:
        _pipeline = TryOnPipeline(sdxl_model_id=settings.sdxl_model_id, controlnet_model_id=settings.controlnet_model_id)
    return _pipeline
...
def image_generation_node(state: AgentState) -> AgentState:
    ...
    generated = pipeline.generate(garment_image=garment_normalized, pose_image=pose_image, guideline=guideline, inpaint_mask=inpaint_mask, num_images=num_images, steps=30, guidance_scale=7.5, controlnet_conditioning_scale=0.8)

    settings.ensure_dirs()
    sku = guideline.brand.lower().replace(" ", "-")
    output_paths = pipeline.save_results(generated, settings.output_dir, prefix=f"stylesync_{sku}")

    return {**state, "garment_clean": garment_clean, "garment_normalized": garment_normalized, "pose_image": pose_image, "inpaint_mask": inpaint_mask, "generated_images": generated, "output_paths": [str(p) for p in output_paths]}
