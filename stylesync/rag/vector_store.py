...
class ProductVectorStore:
    ...
    def upsert_product(self, product_id: str, metadata: GarmentMetadata) -> None:
        ...
        self._index.upsert(
            vectors=[{"id": product_id, "values": embedding, "metadata": metadata.model_dump()}],
            namespace=NAMESPACE,
        )
    ...
    def search(self, query_text: str, top_k: int = 5, brand_filter: Optional[str] = None, category_filter: Optional[str] = None) -> list[dict]:
        ...
        results = self._index.query(
            ...
            namespace=NAMESPACE, filter=filters if filters else None,
        )
        return results.get("matches", [])

    def get_style_guideline(self, query_text: str, brand: Optional[str] = None, category: Optional[str] = None) -> StyleGuideline:
        ...
        if not matches:
            ...
            return StyleGuideline(
                brand=brand or "unknown", pose="front", background="white",
                lighting="studio-soft", style_notes="", has_logo=False,
                logo_position=None, pattern="solid", fit="regular",
            )
        ...
        return StyleGuideline(
            brand=meta.get("brand", "unknown"), pose=meta.get("model_pose", "front"),
            background=meta.get("background", "white"), lighting=meta.get("lighting", "studio-soft"),
            style_notes=meta.get("style_notes", ""), has_logo=meta.get("has_logo", False),
            logo_position=meta.get("logo_position"), pattern=meta.get("pattern", "solid"),
            fit=meta.get("fit", "regular"), negative_prompt_hints=negative_hints,
        )
    ...
    def seed_demo_data(self) -> None:
        ...
        samples = {
            "SKU-NIKE-001": GarmentMetadata(
                ...
                style_notes="Nike brand guidelines: Swoosh logo must remain sharp and unaltered. Use clean white or light-grey studio background. Model should face camera with relaxed posture, hands at sides.",
                model_pose="front", background="white", lighting="studio-soft", sku="SKU-NIKE-001",
            ),
            "SKU-ZARA-042": GarmentMetadata(
                brand="Zara", category="t-shirt", sub_category="v-neck",
                color_primary="white", pattern="solid", fabric="cotton-blend", fit="slim",
                gender="womens", has_logo=False,
                style_notes="Zara editorial style: minimalist background, neutral tones. Model should have a natural, editorial pose. Slight shadow for depth.",
                model_pose="3/4", background="gradient", lighting="natural", sku="SKU-ZARA-042",
            ),
            "SKU-SUPREME-007": GarmentMetadata(
                brand="Supreme", category="t-shirt", sub_category="crew-neck",
                color_primary="red", pattern="graphic", fabric="cotton", fit="oversized",
                gender="unisex", has_logo=True, logo_position="center-chest",
                style_notes="Supreme box logo must be pixel-perfect — no warping or color shift. Streetwear aesthetic: urban background acceptable. Model can have a relaxed, slightly angled stance.",
                model_pose="3/4", background="lifestyle", lighting="dramatic", sku="SKU-SUPREME-007",
            ),
        }
