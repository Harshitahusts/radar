...
def test_build_prompt_basic():
    guideline = StyleGuideline(
        brand="Nike", pose="front", background="white", lighting="studio-soft",
        style_notes="Swoosh must be sharp", has_logo=True, logo_position="center-chest",
        pattern="logo", fit="regular", negative_prompt_hints=["do not alter the logo"],
    )
...
def test_build_prompt_no_logo():
    guideline = StyleGuideline(
        brand="Zara", pose="3/4", background="gradient", lighting="natural",
        style_notes="", has_logo=False, logo_position=None, pattern="solid", fit="slim",
    )
    pos, neg = TryOnPipeline.build_prompt(guideline)
...
def test_build_prompt_lifestyle():
    guideline = StyleGuideline(
        ...
        style_notes="Streetwear aesthetic", has_logo=True, logo_position="center-chest",
        pattern="graphic", fit="oversized", negative_prompt_hints=["no warping", "no color shift"],
    )
    pos, neg = TryOnPipeline.build_prompt(guideline)
